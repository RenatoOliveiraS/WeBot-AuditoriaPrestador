...
def montar_comparacao_guias(report_rows: list[dict], xlsx_path: str) -> list[dict]:
    ...
    for row in report_rows:
        ...
        modulo_tem_valor = is_gt_zero(esperado_pis, TOLERANCIA_VALOR) or is_gt_zero(esperado_cofins, TOLERANCIA_VALOR)

        if guia_xlsx:
            xlsx_pis = to_dec(guia_xlsx.get("pis"))
            xlsx_cofins = to_dec(guia_xlsx.get("cofins"))
            xlsx_tem_valor = is_gt_zero(xlsx_pis, TOLERANCIA_VALOR) or is_gt_zero(xlsx_cofins, TOLERANCIA_VALOR)

            if modulo_tem_valor and not xlsx_tem_valor:
                status_comp = "GUIA_NAO_GERADA_XLSX"
                obs = "Módulo possui valores, mas o relatório XLSX está zerado (guia não gerada/consolidada)."
            elif (not modulo_tem_valor) and (not xlsx_tem_valor):
                status_comp = "SEM_MOVIMENTO"
                obs = "Valores zerados no módulo e no XLSX; guia não esperada para a empresa."
            else:
            ...
            if modulo_tem_valor:
                status_comp = "GUIA_NAO_LOCALIZADA_XLSX"
                obs = "Empresa não encontrada no XLSX, porém o módulo possui valores de guia."
            else:
                status_comp = "SEM_MOVIMENTO_SEM_XLSX"
                obs = "Empresa não encontrada no XLSX e valores do módulo zerados; guia não esperada."
